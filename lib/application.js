...
app.init = function(){
  ...
  this.viewCallbacks = [];
  this.paramCallbacks = [];
  this.setupImplicitMiddleware();
  this.setupLocals();
  this.setupDefaultConfiguration();
};

/**
 * Initialize implicit middleware.
 *
 * @api private
 */

app.setupImplicitMiddleware = function(){
...
app.setupDefaultConfiguration = function(){
  var self = this;
...
app.param = function(name, fn){
  ...
  // single
    ...
    fns.forEach(function(fn){
      (self.paramCallbacks[name] = self.paramCallbacks[name] || []).push(fn);
    });
  }
...
/**
 * Register route `method` and `path` to
...
app.route = function(method, path, fn){
  var self = this
    , route = new Route(method, path)
    , params = this.paramCallbacks;

  // param callbacks
  route.keys.forEach(function(key){
    var fns = params[key.name];
    if (!fns) return;
    fns.forEach(function(fn){
      self.use(route.middleware(function(req, res, next){
        var args = [req, res, next, req.params[key.name]];
        fn.apply(null, args);
      }));
    });
  });

  this.use(route.middleware(fn));
  return this;
};
